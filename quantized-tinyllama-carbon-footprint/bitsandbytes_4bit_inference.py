...
print("TinyLlama loaded successfully using BitsAndBytes 4-bit (NF4).")

results = []

tracker = EmissionsTracker(
    output_file="llama_bnb_4bit_codecarbon_logs.csv",
    save_to_file=True
)
...
results_df.to_csv(
    "llama_bnb_4bit_experiment_results.csv",
    index=False
)

results_df.head()

logs = pd.read_csv("llama_bnb_4bit_codecarbon_logs.csv")

summary = pd.DataFrame({
    "Model": ["TinyLlama BitsAndBytes 4-bit"],
...
summary.to_csv(
    "llama_bnb_4bit_carbon_emissions_report.csv",
    index=False
)
